...
__global__ void apply_delta_kernel(int samples, const void* anchors, const void* delta, void* outputBbox)
{
    ...
    for (int i = 0; i < totalItems; i++)
    {
        ...
        if (cur_id < samples)
        {
            ...
            cur_anchor_yxyx.y2 = cur_anchor_cyxhw.y1 + 0.5 * cur_anchor_cyxhw.y2;
            cur_anchor_yxyx.x2 = cur_anchor_cyxhw.x1 + 0.5 * cur_anchor_cyxhw.x2;
...
__global__ void roiAlign_kernel(int featureLenght, int roiCount,
...
cudaError_t roiAlign(cudaStream_t stream, int batchSize, int featureLenght, int roiCount, float firstThreshold,
    ...
    void* pooled, const xy_t poolDims, const xy_t inputImageSize)//Batch*roiCount(1000)*featureLenght(256)*7*7 通过permute变成Batch*featureLenght*roiCount*y*x，不转置也没事，本质是一样的
{
    ...
    roiAlign_kernel<<<blocks, threads, 0, stream>>>(featureLenght, roiCount, firstThreshold,
