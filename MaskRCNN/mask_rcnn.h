/* implentation of mask_rcnn 
 *
 *
 */

#ifndef MASKRCNN_MASKRCNN_H
#define MASKRCNN_MASKRCNN_H
...
static constexpr int poolSize_Mask = 14;
static constexpr int featureMapChannels = 256;
static constexpr int inputSize_H = 800;
static constexpr int inputSize_W = 800;
...
const std::string gSampleName = "TensorRT.MaskRCNN";


struct MaskRCNNParams : public samplesCommon::OnnxSampleParams
{
...
public:
    MaskRCNN(const MaskRCNNParams& params)
        : mParams(params)
        , mEngine(nullptr)
    {
    }
...
private:
    MaskRCNNParams mParams; //!< The parameters for the sample.

    nvinfer1::Dims mInputDims; //!< The dimensions of the input to the network.

    static const int kIMG_CHANNELS = 3;
    static const int kIMG_H = 800;
    static const int kIMG_W = 800;
    ...
    std::vector<ITensor*>  roiAlignBuild_Mask(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, std::vector<ITensor*>& inputTensors);
    std::vector<ITensor*> maskBuild(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, ITensor* roiAlignTensor);
    ITensor* decodeRCNNOut(SampleUniquePtr<nvinfer1::INetworkDefinition>& network,std::vector<ITensor*> rcnn_results, ITensor* rois);
