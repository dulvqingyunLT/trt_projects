...
bool read_cache)
    ...
    LGT::spanner::read_files_in_dir(img_dir, img_files_);
}
...
bool Int8EntropyCalibrator2::getBatch(void* bindings[], const char* names[], int nbBindings) {
    ...
    // std::vector<float> imageBuffer(input_h_ * input_w_ * 3, 0);
    for (int i = img_idx_; i < img_idx_ + batchsize_; i++) {
        std::cout << img_files_[i] << "  " << i << std::endl;
        cv::Mat matOut = cv::imread(img_dir_ + img_files_[i]);
        matOut = LGT::spanner::PreProcess::resize_to_bgrmat_with_ratio(matOut, input_h_, input_w_);
        if (matOut.empty()) {
        ...
        for (int c = 0; c < 3; ++c)//
        {
           ...
           for (unsigned j = 0, volChl = input_h_ * input_w_; j < volChl; ++j)  //将HW看成一维
            //    imageBuffer[ c * volChl + j] = data[j * 3 + 2 - c];//   相当一个二维数组转置 2-c是BGR->RGB

               input_imgs_[(i-img_idx_)*input_w_*input_h_*3 + c * volChl + j] = data[j * 3 + 2 - c];
        }

        // for (int ind = 0; ind < input_w_*input_h_*3; ind++)
        //     input_imgs_[(i-img_idx_)*input_w_*input_h_*3 + ind] = imageBuffer[ind];
    }
    img_idx_ += batchsize_;
