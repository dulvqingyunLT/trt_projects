...
/* 直接加载roialign输出*/
    // auto roiAlignOutFile = locateFile("roi_align_out.wts", mParams.dataDirs);
    // std::map<std::string, Weights> roiAlignOut = LGT::CV::loadWeights(roiAlignOutFile);

    // auto roi_align_constant_layer = network->addConstant(Dims4{1000, 256, 7, 7 }, roiAlignOut["roi_align_tensor"]);
    // assert(roi_align_constant_layer);


    // IFullyConnectedLayer* share_fc_0_layer = network->addFullyConnected(*roi_align_constant_layer->getOutput(0),
    ...
    // auto* relu_0_reshape_layer = network->addShuffle(*relu_0_layer->getOutput(0));
    // relu_0_reshape_layer->setReshapeDimensions(Dims4{-1, 1, 1,1024}); 
...
/**3维softmax**/
    ...
    reshapeLayer_scores->setReshapeDimensions(
           Dims3{-1, mParams.roiCount, numClasses+1});          
    // auto dim_output_reshapeLayer_scores = reshapeLayer_scores->getOutput(0)->getDimensions();
    ...
    // cls_layer->getOutput(0)->setName("cls_out");
    // network->markOutput(*cls_layer->getOutput(0));
    ...
    // roi_align_constant_layer->getOutput(0)->setName("roi_align_out");
    // network->markOutput(*roi_align_constant_layer->getOutput(0));

    // share_fc_0_layer->getOutput(0)->setName("share_fc_0_out");
    // network->markOutput(*share_fc_0_layer->getOutput(0));

    // relu_0_layer->getOutput(0)->setName("relu_0_out");
    // network->markOutput(*relu_0_layer->getOutput(0));
    
    // share_fc_1_layer->getOutput(0)->setName("share_fc_1_out");
    // network->markOutput(*share_fc_1_layer->getOutput(0));

    // relu_1_layer->getOutput(0)->setName("relu_1_out");
    // network->markOutput(*relu_1_layer->getOutput(0));
...
bool FasterRCNN::verifyOutput(const samplesCommon::BufferManager& buffers)
{
    ...
    //         {
    //              max_score =  cls_out[i*(numClasses+1)+j];
    //              max_index = j;
    //         }
    //         std::cout<<cls_out[i*(numClasses+1)+j]<<" ";
    //     }
        
    //     std::cout<<"index: "<<max_index<<" "<<max_score<<std::endl;        

    // }  

    // float* roi_align_out = static_cast<float*>(buffers.getHostBuffer("roi_align_out"));

    // float* share_fc_0_out = static_cast<float*>(buffers.getHostBuffer("share_fc_0_out")); 

    // float* relu_0_out = static_cast<float*>(buffers.getHostBuffer("relu_0_out"));
    // for(int i =0; i< 1000; i++)
    // {

    //     for(int j = 0; j<1024; j++)
    //     {  
    //         std::cout<<cls_out[i*1024+j]<<" ";
    //     }
        
    //     std::cout<<std::endl;        

    // } 
    // float* share_fc_1_out = static_cast<float*>(buffers.getHostBuffer("share_fc_1_out"));


    // float* relu_1_out = static_cast<float*>(buffers.getHostBuffer("relu_1_out"));    
...
/***
    float* softmax_out = static_cast<float*>(buffers.getHostBuffer("softmax_out"));    //1,1000,81   
    ...
    for(int i =0; i< mParams.roiCount; i++)
    {
        ...
        int max_index = -1;
        for(int j = 0; j<(numClasses+1); j++)
        {   if(max_score <= softmax_out[i*(numClasses+1)+j])
            {
                 max_score =  softmax_out[i*(numClasses+1)+j];
                 max_index = j;
            }
    ...
    for(int i =0; i< mParams.roiCount; i++)
    {
        ...
        for(int j = 0; j<numClasses; j++)
        {
            ...
            {
                max_score =  slice_cls_out_0[i*numClasses+j];
                max_index = j;
        ...
        std::cout<<"index: "<<max_index<<" "<<max_score<<std::endl;
        score_index_vec.push_back(std::make_pair(max_score, max_index));       

    }
    std::sort(score_index_vec.begin(), score_index_vec.end(), 
    [](std::pair<float,int>e1, std::pair<float,int>e2){return e1.first>e2.first;});
***/
...
/***
    ...
    for(int i = 0; i < mParams.batchSize; i++)
    {
        for ( int j = 0; j< numClasses; j++)
        {
            ...
            {
                class_bbox.push_back( bbox[i*mParams.roiCount*numClasses + k* numClasses +j] );
            }
