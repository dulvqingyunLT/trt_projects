...
{
    // mPluginAttributes.emplace_back(PluginField("target_mean", nullptr, PluginFieldType::kFLOAT32, 4));
    // mPluginAttributes.emplace_back(PluginField("target_std", nullptr, PluginFieldType::kFLOAT32, 4));
...
IPluginV2DynamicExt* DynamicSliceBackgroundPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    ...
    //     if (!strcmp(attrName, "target_mean"))
    //     {
    //         assert(fields[i].type == PluginFieldType::kFLOAT32);
    //         mTargetMean = *(static_cast<const quad_t*>(fields[i].data));
    //     }
    //     if (!strcmp(attrName, "target_std"))
    //     {
    //         assert(fields[i].type == PluginFieldType::kFLOAT32);
    //         mTargetStd = *(static_cast<const quad_t*>(fields[i].data));
    //     }
    // }
...
void DynamicSliceBackground::serialize(void* buffer) const
{
    ...
    // write(d, mTargetMean[0]);
    // write(d, mTargetMean[1]);
...
DynamicSliceBackground::DynamicSliceBackground(const void* data, size_t length)
{
    ...
    // mTargetMean[0] = read<float>d;
    // mTargetMean[1] = read<float>d;
