...
bool MaskRCNN::build()
{
    ...
    if(mParams.saveEngine)
    {
       ...
       std::ofstream p("Mask_RCNN.plan", std::ios::binary);
       if (!p)
       {
           return false;
       }
...
std::vector<ITensor*>  MaskRCNN::roiAlignBuild(SampleUniquePtr<nvinfer1::INetworkDefinition>& network){
...
std::vector<ITensor*> MaskRCNN:: rcnnBuild(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, ITensor* roiAlignTensor)
 {
...
std::vector<ITensor*>  MaskRCNN::roiAlignBuild_Mask(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, std::vector<ITensor*>& inputTensors){
    
    // ITensor* roi = inputTensors[0];
    // ITensor* fp0 = network->getOutput(2);  //[1,256,*,*]
    // ITensor* fp1 = network->getOutput(3);
    // ITensor* fp2 = network->getOutput(4);
    // ITensor* fp3 = network->getOutput(5);
    ...
    roialign_attr.emplace_back(PluginField("pooled_size",  &poolSize_Mask, PluginFieldType::kINT32, 1));
    roialign_attr.emplace_back(PluginField("input_size_h", &inputSize_H, PluginFieldType::kINT32, 1));
    roialign_attr.emplace_back(PluginField("input_size_w", &inputSize_W, PluginFieldType::kINT32, 1));
    ...
    auto pluginObj = creator->createPlugin("CustomizedPyramidROIAlign",pluginROIFC.get());

    ITensor* inputROIAlignTensors[] = {inputTensors[0], inputTensors[1], inputTensors[2], inputTensors[3], inputTensors[4]};

    IPluginV2Layer* PyramidROIAlign_layer = network->addPluginV2(inputROIAlignTensors, 5, *pluginObj);
    auto dim_output_roiAlign = PyramidROIAlign_layer->getOutput(0)->getDimensions();

    PyramidROIAlign_layer->getOutput(0)->setName("roiAlign_mask");
    network->markOutput(*PyramidROIAlign_layer->getOutput(0));
...
std::vector<ITensor*> MaskRCNN:: maskBuild(SampleUniquePtr<nvinfer1::INetworkDefinition>& network, ITensor* roiAlignTensor)
 {
    auto rcnnWeightFile = locateFile(mParams.rcnnWeightsFile, mParams.dataDirs);
    std::map<std::string, Weights> weightMap = LGT::spanner::loadWeights(rcnnWeightFile);
    
    IConvolutionLayer* share_conv_0_layer = network->addConvolutionNd(*roiAlignTensor,
                                                                       256,
                                                                       DimsHW{3, 3},
                                                                       weightMap["mask_head.convs.0.conv.weight"],
                                                                       weightMap["mask_head.convs.0.conv.bias"]);
    assert(share_conv_0_layer);
    share_conv_0_layer->setStrideNd(DimsHW{1, 1});
    share_conv_0_layer->setPaddingNd(DimsHW{1, 1});

    auto dim_output_conv_0 = share_conv_0_layer->getOutput(0)->getDimensions();
    IActivationLayer* relu_0_layer = network->addActivation(*share_conv_0_layer->getOutput(0), ActivationType::kRELU);

    IConvolutionLayer* share_conv_1_layer = network->addConvolutionNd(*relu_0_layer->getOutput(0),
                                                                       256,
                                                                       DimsHW{3, 3},
                                                                       weightMap["mask_head.convs.1.conv.weight"],
                                                                       weightMap["mask_head.convs.1.conv.bias"]);
    assert(share_conv_1_layer);
    share_conv_1_layer->setStrideNd(DimsHW{1, 1});
    share_conv_1_layer->setPaddingNd(DimsHW{1, 1});
    IActivationLayer* relu_1_layer = network->addActivation(*share_conv_1_layer->getOutput(0), ActivationType::kRELU);

    IConvolutionLayer* share_conv_2_layer = network->addConvolutionNd(*relu_1_layer->getOutput(0),
                                                                       256,
                                                                       DimsHW{3, 3},
                                                                       weightMap["mask_head.convs.2.conv.weight"],
                                                                       weightMap["mask_head.convs.2.conv.bias"]);
    assert(share_conv_2_layer);
    share_conv_2_layer->setStrideNd(DimsHW{1, 1});
    share_conv_2_layer->setPaddingNd(DimsHW{1, 1});
    IActivationLayer* relu_2_layer = network->addActivation(*share_conv_2_layer->getOutput(0), ActivationType::kRELU);

    IConvolutionLayer* share_conv_3_layer = network->addConvolutionNd(*relu_2_layer->getOutput(0),
                                                                       256,
                                                                       DimsHW{3, 3},
                                                                       weightMap["mask_head.convs.3.conv.weight"],
                                                                       weightMap["mask_head.convs.3.conv.bias"]);
    assert(share_conv_3_layer);
    share_conv_3_layer->setStrideNd(DimsHW{1, 1});
    share_conv_3_layer->setPaddingNd(DimsHW{1, 1});
    IActivationLayer* relu_3_layer = network->addActivation(*share_conv_3_layer->getOutput(0), ActivationType::kRELU);

    IDeconvolutionLayer* share_upsample_layer = network->addDeconvolutionNd(*relu_3_layer->getOutput(0),
                                                                            256,
                                                                            DimsHW{2, 2},
                                                                            weightMap["mask_head.upsample.weight"],
                                                                            weightMap["mask_head.upsample.bias"]);
    assert(share_upsample_layer);
    share_upsample_layer->setStrideNd(DimsHW{2, 2});
   
    IActivationLayer* relu_4_layer = network->addActivation(*share_upsample_layer->getOutput(0), ActivationType::kRELU);

    IConvolutionLayer* share_conv_logits_layer = network->addConvolutionNd(*relu_4_layer->getOutput(0),
                                                                       80,
                                                                       DimsHW{1, 1},
                                                                       weightMap["mask_head.conv_logits.weight"],
                                                                       weightMap["mask_head.conv_logits.bias"]);
    assert(share_conv_logits_layer);
    share_conv_logits_layer->setStrideNd(DimsHW{1, 1});
    
    IActivationLayer* relu_logits_layer = network->addActivation(*share_conv_logits_layer->getOutput(0), ActivationType::kRELU);


    std::vector<ITensor*> instances;
    instances.push_back(relu_logits_layer->getOutput(0));
    relu_logits_layer->getOutput(0)->setName("masks_out");
    network->markOutput(*relu_logits_layer->getOutput(0));
...
ITensor* MaskRCNN:: decodeRCNNOut(SampleUniquePtr<nvinfer1::INetworkDefinition>& network,std::vector<ITensor*> rcnn_results, ITensor* nmsed_rpns)
{
   ...
   // add detla2bbox plugin
    ...
    return NMS_layer->getOutput(1);  
}

bool MaskRCNN::constructNetwork(SampleUniquePtr<nvonnxparser::IParser>& parser,
    ...
    SampleUniquePtr<nvinfer1::IBuilderConfig>& config)
{
...

   {
    ...
    //    samplesCommon::setAllTensorScales(network.get(), 127.0f, 127.0f);
        ...
        Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(mParams.batchSize, inputSize_W, inputSize_H, "/home/yhuang/datasets/coco/images/val2017/",
        "int8calib_maskrcnn.table", input->getName());
        config->setInt8Calibrator(calibrator);   

   }

    builder->setMaxBatchSize(mParams.batchSize);
    config->setMaxWorkspaceSize(1ULL << 32);  //4_GiB or 8_GiB or 1ULL << 32
    ...
    auto nms_bboxs = decodeRCNNOut(network,rcnn_results, roiAlign_results[0]);

    std::vector<ITensor*> inputTesnors{nms_bboxs, fp0, fp1, fp2, fp3};
    auto roiAlign_mask_results = roiAlignBuild_Mask(network, inputTesnors);
    auto rcnn_mask_results = maskBuild(network, roiAlign_mask_results[0]);
...
bool MaskRCNN::infer()
{
    ...
    SimpleProfiler profiler("MaskRCNN performance");
    if (mParams.profile)
    {
...
/***
    ...
    // Fill data buffer
    float* hostDataBuffer = static_cast<float*>(buffers.getHostBuffer("input"));
    // Pixel mean used by the Mask R-CNN's author
...
//!
bool MaskRCNN::verifyOutput(const samplesCommon::BufferManager& buffers)
{
    const int batchSize = mParams.batchSize;
...
***/
    ...
    float* masks_out = static_cast<float*>(buffers.getHostBuffer("masks_out"));
...
//!
void MaskRCNN::bboxTransformInvAndClip(const float* rois, const float* deltas, float* predBBoxes,
...
//!
std::vector<int> MaskRCNN::nonMaximumSuppression(std::vector<std::pair<float, int>>& scoreIndex, float* bbox,
